...
module.exports = class Cucumber{
    ...
    async report(){
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            fs.writeFile( reportPath , JSON.stringify([this.reportFeature(feature)]), err => {
                if(err) {
    ...
    reportFeature(feature){
        ...
        forEachRule(feature, rule => {
            forEachScenarioIn(rule, (scenario, examples) => {
                let scenarioReportObj = this.reportScenario(scenario);
                // if(examples){
                //     scenarioReportObj.examples = this.reportExamples(examples);
                // }
    ...
    reportScenario(scenario){
        ...
        forEachStep(scenario, step => {
            if(step.status !== "skipped") scenarioReportObj.steps.push( this.reportStep(step) );
        })
    ...
    reportStep(step){
        ...
        this.addEmbeded(step, stepReportObj);
        if(step.arg){
